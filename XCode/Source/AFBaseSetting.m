...
-(void)addObserver:(NSObject<AFSettingObserver>*)observer{[settingObservers addObject:observer];}
-(void)removeObserver:(NSObject<AFSettingObserver>*)observer{[settingObservers removeObject:observer];}


-(void)controlValueChanged:(id)sender{[self doesNotRecognizeSelector:_cmd];}
...
-(void)setValue:(NSObject<NSCoding>*)valueIn
{
	...
	if(value!=valueIn)
	{
		...
		for(NSObject<AFSettingObserver>* observer in settingObservers){[observer settingChanged:(NSObject<AFSetting>*)self];}
...
-(NSObject<AFValidator>*)validator{return validator;}
...
{
	return self.parentSection.parentTable.parentController;
}

+(UIColor*)validColor
{
	if(!validColor){validColor = [[[AFThemeManager themeSectionForClass:[AFBaseSetting class]] colorForKey:THEME_KEY_VALID_COLOR] retain];}
	return validColor;
}

+(UIColor*)invalidColor
{
	if(!invalidColor){invalidColor = [[[AFThemeManager themeSectionForClass:[AFBaseSetting class]] colorForKey:THEME_KEY_INVALID_COLOR] retain];}
	return invalidColor;
}
...
+(NSString*)themeSectionName{return @"setting";}
