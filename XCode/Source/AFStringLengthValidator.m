...
-(BOOL)isValid:(NSObject*)testObject
{
	switch(mode)
	{
		case EXACTLY:  return [(NSString*)testObject length]==length;
		case AT_LEAST: return [(NSString*)testObject length]>=length;
		case AT_MOST:  return [(NSString*)testObject length]<=length;
		default: break;
	}
	return NO;
}
