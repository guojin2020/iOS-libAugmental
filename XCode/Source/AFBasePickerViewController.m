...
-(NSInteger)pickerView:(UIPickerView*)pickerView numberOfRowsInComponent:(NSInteger)component{return component==0?[objects count]:0;}
...
+(UIColor*)textColor
{
	if(!textColor){textColor = [[[AFThemeManager themeSectionForClass:[AFBasePickerViewController class]] colorForKey:THEME_KEY_TEXT_COLOR] retain];}
	return textColor;
}

+(UIColor*)bgColor
{
	if(!bgColor){bgColor = [[[AFThemeManager themeSectionForClass:[AFBasePickerViewController class]] colorForKey:THEME_KEY_BG_COLOR] retain];}
	return bgColor;
}
...
+(NSString*)themeSectionName{return @"picker";}
