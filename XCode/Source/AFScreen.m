...
-(BOOL)viewControllerIsLoaded{return viewController!=nil;}
...
-(UIImage*)tabBarIcon{return [[self class] performSelector:@selector(defaultTabBarIcon)];}

-(NSArray*)settingsSections{return nil;}

-(void)settingValueChanged:(NSObject<AFSetting>*)setting{[self doesNotRecognizeSelector:_cmd];}

-(void)refreshActive{[self doesNotRecognizeSelector:_cmd];}
...
-(void)addScreenObserver:(NSObject<AFScreenObserver>*)observer{[observers addObject:observer];}
-(void)removeScreenObserver:(NSObject<AFScreenObserver>*)observer{[observers removeObject:observer];}
...
-(UINavigationController*)navigationController{return viewController.navigationController;}
...
-(UIImage*)defaultTabBarIcon{return [[self class] performSelector:@selector(defaultTabBarIcon)];}
-(NSString*)defaultTitle{return [[self class] performSelector:@selector(defaultTitle)];}
-(NSString*)defaultTabName{return [[self class] performSelector:@selector(defaultTabName)];}

+(UIImage*)defaultTabBarIcon{return nil;}
+(NSString*)defaultTitle{return @"Untitled";}
+(NSString*)defaultTabName{return @"Untitled";}

//==============>> Theme Getters

+(UIColor*)bgColor
{
	if(!bgColor){bgColor=[[[AFThemeManager themeSectionForClass:[AFScreen class]] colorForKey:THEME_KEY_LOADING_BG_COLOR] retain];}
	return bgColor;
}

+(float)bgOpacity
{
	if(!bgOpacity){bgOpacity=[[[AFThemeManager themeSectionForClass:[AFScreen class]] objectForKey:THEME_KEY_LOADING_BG_OPACITY] retain];}
	return [bgOpacity floatValue];
}

+(NSString*)loadingTitle
{
	if(!loadingTitle){loadingTitle=[[[AFThemeManager themeSectionForClass:[AFScreen class]] valueForKey:THEME_KEY_LOADING_TITLE] retain];}
	return loadingTitle;
}

+(UIColor*)loadingTitleColor
{
	if(!loadingTitleColor){loadingTitleColor=[[[AFThemeManager themeSectionForClass:[AFScreen class]] colorForKey:THEME_KEY_LOADING_TITLE_COLOR] retain];}
...
+(NSString*)themeSectionName{return @"screen";}
