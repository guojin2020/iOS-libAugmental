...
-(id)init
{
	...
	{
		loadingView = nil;
		self.observers = [[NSMutableSet alloc] initWithCapacity:2];
	}
	return self;
}
...
-(void)addObserver:(NSObject<AFSettingViewPanelObserver>*)observerIn{[observers addObject:observerIn];}
-(void)removeObserver:(NSObject<AFSettingViewPanelObserver>*)observerIn{[observers removeObject:observerIn];}

-(void)broadcastNewValueToObservers:(NSObject*)newValue
{
	for(NSObject<AFSettingViewPanelObserver>* observer in observers){[observer settingViewPanel:self valueChanged:newValue];}
}
