...
-(BOOL)actionRequest:(NSObject<AFRequest>*)request{return [targetHandler handleRequest:request];}

/**
 *	Cancels all requests in this queue
 */
-(void)cancelAllRequests{for(NSObject<AFQueueableRequest>* request in [NSArray arrayWithArray:queue])if(request)[request cancel];}
...
-(BOOL)isRequestActive:(NSObject<AFRequest>*)request{return [activeRequests containsObject:request];}
