...
-(void)screenBecameInactive:(AFScreen*)screen{[self hideScreen:screen];}
-(void)screenBecameActive:(AFScreen*)screen{[self showScreen:screen];}
...
{
	if(!bgImageName){bgImageName=[[[AFThemeManager themeSectionForClass:[AFScreenManager class]] valueForKey:THEME_KEY_BG_IMAGE_NAME] retain];}
	return bgImageName;
}

+(BOOL)bgImageEnable
{ 
	if(!bgImageEnable){bgImageEnable=[[[AFThemeManager themeSectionForClass:[AFScreenManager class]] valueForKey:THEME_KEY_BG_IMAGE_ENABLE] retain];}
	return [bgImageEnable boolValue];
}

+(UIColor*)bgColor
{
	if(!bgColor){bgColor=[[[AFThemeManager themeSectionForClass:[AFScreenManager class]] colorForKey:THEME_KEY_BG_COLOR] retain];}
	return bgColor;
}
...
+(NSString*)themeSectionName{return @"screenManager";}
